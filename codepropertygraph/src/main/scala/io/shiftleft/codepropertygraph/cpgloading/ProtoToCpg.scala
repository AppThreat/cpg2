...
import java.util.{Collection as JCollection, NoSuchElementException}
import scala.jdk.CollectionConverters.*

object ProtoToCpg:

    def toProperty(keyValue: (String, PropertyValue))(implicit
    ...
    private def toRegularType(value: PropertyValue)(implicit interner: StringInterner): Any =
        value.getValueCase match
            ...
            case STRING_LIST =>
                value.getStringList.getValuesList.asScala.map(interner.intern).toList
            case VALUE_NOT_SET => ()
            case _ => throw new RuntimeException(
                  "Error: unsupported property case: " + value.getValueCase.name
                )
...
class ProtoToCpg(overflowConfig: Config = Config.withoutOverflow):
    ...
    private def findNodeById(nodeId: Long, typ: EdgeType): overflowdb.Node =
        if nodeId == -1 then
            throw new IllegalArgumentException(
              "Illegal src|dst node ID -1 on edge of type " + typ.name
            )
        odbGraph
            .nodeOption(nodeId)
            .getOrElse(
