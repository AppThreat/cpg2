...
object CpgLoader:
    ...
    /** Load Code Property Graph from an overflow DB file
      ...
      *   loader config
      *
      * This methods loads the CPG from an existing overflow DB file, specified in
      * config.overflowDbConfig. In particular, this config specifies the filename. For example, to
      * load the database at "foo.db", you can issue the following:
      *
...
private class CpgLoader:
    ...
    def load(filename: String, config: CpgLoaderConfig = CpgLoaderConfig.withoutOverflow): Cpg =
        ...
        if config.createIndexes then createIndexes(cpg)
        cpg
