...
/** A pool of long integers that serve as node ids. Using the method `next`, the pool provides the
...
class IntervalKeyPool(val first: Long, val last: Long) extends KeyPool:
    ...
    /** Split key pool into `numberOfPartitions` partitions of mostly equal size. Invalidates the
      * current pool to ensure that the user does not continue to use both the original pool and
      * pools derived from it via `split`.
      */
