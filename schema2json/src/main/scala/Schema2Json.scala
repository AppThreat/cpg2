    ...
    private def schemaSummary =
        (schema.nodeTypes.map(_.schemaInfo) ++ schema.edgeTypes.map(_.schemaInfo))
            ...
            .map { info =>
                ...
                val description = info.getDeclaringClass.getDeclaredMethod("description").invoke(
                  null
                ).asInstanceOf[String]
                val providedByFrontend =
                    info.getDeclaringClass.getDeclaredMethod("providedByFrontend").invoke(
                      null
        ...
        (schema.nodeTypes ++ schema.nodeBaseTypes)
            ...
            .flatMap { nodeType =>
                ...
                val allPropertyNames =
                    nodeType.properties.filterNot(x => isSchemaHidden(x.schemaInfo)).map(_.name)
                val cardinalities =
                    nodeType.properties.filterNot(x => isSchemaHidden(x.schemaInfo)).map(p =>
                        name(p.cardinality)
                    )
                ...
                val nonInheritedProperties =
                    allPropertyNames.filterNot(x => inheritedPropertyNames.contains(x))
