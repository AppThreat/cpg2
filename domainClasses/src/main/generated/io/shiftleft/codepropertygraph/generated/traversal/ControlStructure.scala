...
class ControlStructureTraversalExtGen[NodeType <: ControlStructure](
  val traversal: Iterator[NodeType]
    ...
    def argumentName(pattern: String): Iterator[NodeType] =
        if !Misc.isRegex(pattern) then
            traversal.filter { node =>
                node.argumentName.isDefined && node.argumentName.get == pattern
            }
        else
            overflowdb.traversal.filter.StringPropertyFilter.regexp(
              traversal.filter(_.argumentName.isDefined)
            )(_.argumentName.get, pattern)
    ...
    def argumentNameNot(pattern: String): Iterator[NodeType] =
        if !Misc.isRegex(pattern) then
            traversal.filter { node =>
                node.argumentName.isEmpty || node.argumentName.get != pattern
            }
        else
    ...
    /** Traverse to nodes where the controlStructureType matches at least one of the regular
    ...
    /** Traverse to nodes where controlStructureType does not match any of the regular expressions
      * in `values`.
      */
    ...
    /** Traverse to nodes where the parserTypeName matches at least one of the regular expressions
      * in `values`
      */
...
end ControlStructureTraversalExtGen
