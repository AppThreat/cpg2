...
class UnknownTraversalExtGen[NodeType <: Unknown](val traversal: Iterator[NodeType]) extends AnyVal:
    ...
    /** Traverse to nodes where the containedRef matches at least one of the regular expressions in
      * `values`
      */
    ...
    /** Traverse to nodes where containedRef does not match any of the regular expressions in
      * `values`.
      */
