...
class MethodRef(graph_4762: Graph, id_4762: Long /*cf https://github.com/scala/bug/issues/4762 */ )
    ...
    /** Traverse to ANNOTATION via AST OUT edge.
      */
    def _annotationViaAstOut: overflowdb.traversal.Traversal[Annotation] =
        get()._annotationViaAstOut
    ...
    def _closureBindingViaCaptureOut: overflowdb.traversal.Traversal[ClosureBinding] =
        get()._closureBindingViaCaptureOut
    ...
    /** Traverse to METHOD_RETURN via CFG OUT edge.
      */
    def _methodReturnViaCfgOut: overflowdb.traversal.Traversal[MethodReturn] =
        get()._methodReturnViaCfgOut
    ...
    /** Traverse to RETURN via REACHING_DEF IN edge.
      */
    def _returnViaReachingDefIn: overflowdb.traversal.Traversal[Return] =
        get()._returnViaReachingDefIn
...
class MethodRefDb(ref: NodeRef[NodeDb]) extends NodeDb(ref) with StoredNode
    ...
    def _annotationViaAstOut: overflowdb.traversal.Traversal[Annotation] =
        astOut.collectAll[Annotation]

    def captureOut: Iterator[ClosureBinding] =
        createAdjacentNodeScalaIteratorByOffSet[ClosureBinding](2)
    ...
    def _methodReturnViaCfgOut: overflowdb.traversal.Traversal[MethodReturn] =
        cfgOut.collectAll[MethodReturn]
    ...
    def referencedMethod: Method =
        try refOut.collectAll[Method].next()
        catch
    ...
    def conditionIn: Iterator[ControlStructure] =
        createAdjacentNodeScalaIteratorByOffSet[ControlStructure](15)
    ...
    def _returnViaReachingDefIn: overflowdb.traversal.Traversal[Return] =
        reachingDefIn.collectAll[Return]
    def _typeRefViaReachingDefIn: overflowdb.traversal.Traversal[TypeRef] =
    ...
    override def fromNewNode(newNode: NewNode, mapping: NewNode => StoredNode): Unit =
        ...
        this._dynamicTypeHintFullName =
            if newNode.asInstanceOf[NewMethodRef].dynamicTypeHintFullName != null then
        ...
        this._possibleTypes = if newNode.asInstanceOf[NewMethodRef].possibleTypes != null then
            newNode.asInstanceOf[NewMethodRef].possibleTypes
        else collection.immutable.ArraySeq.empty
