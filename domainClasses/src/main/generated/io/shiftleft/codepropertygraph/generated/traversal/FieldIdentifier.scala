...
class FieldIdentifierTraversalExtGen[NodeType <: FieldIdentifier](val traversal: Iterator[NodeType])
    extends AnyVal:
    ...
    /** Traverse to nodes where the canonicalName matches at least one of the regular expressions in
      * `values`
      */
    ...
    /** Traverse to nodes where canonicalName does not match any of the regular expressions in
      * `values`.
      */
    ...
    def orderNot(values: scala.Int*): Iterator[NodeType] =
        val vset = values.toSet
        traversal.filter { node => !vset.contains(node.order) }
end FieldIdentifierTraversalExtGen
