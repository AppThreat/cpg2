...
class KeyValuePairTraversalExtGen[NodeType <: KeyValuePair](val traversal: Iterator[NodeType])
    extends AnyVal:
    ...
    def keyExact(value: String): Iterator[NodeType] =
        val fastResult = traversal match
            case init: overflowdb.traversal.InitialTraversal[NodeType] =>
                init.getByIndex("KEY", value).getOrElse(null)
            case _ => null
    ...
    def valueExact(value: String): Iterator[NodeType] =
        val fastResult = traversal match
            case init: overflowdb.traversal.InitialTraversal[NodeType] =>
                init.getByIndex("VALUE", value).getOrElse(null)
            case _ => null
        if fastResult != null then fastResult
...
end KeyValuePairTraversalExtGen
