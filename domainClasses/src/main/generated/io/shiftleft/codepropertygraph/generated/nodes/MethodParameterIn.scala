...
object MethodParameterIn:
    ...
    object Edges:
        val Out: Array[String] =
            Array("AST", "EVAL_TYPE", "PARAMETER_LINK", "REACHING_DEF", "TAGGED_BY")
...
end MethodParameterIn

trait MethodParameterInBase extends AbstractNode with AstNodeBase with CfgNodeBase
...
class MethodParameterIn(
  ...
  id_4762: Long /*cf https://github.com/scala/bug/issues/4762 */
    ...
    /** Traverse to corresponding formal output parameter Traverse to METHOD_PARAMETER_OUT via
      * PARAMETER_LINK OUT edge.
      */
    ...
    /** Traverse to CLOSURE_BINDING via REF IN edge.
      */
    def _closureBindingViaRefIn: overflowdb.traversal.Traversal[ClosureBinding] =
        get()._closureBindingViaRefIn

    /** Places (identifier) where this parameter is being referenced Traverse to IDENTIFIER via REF
      * IN edge.
      */
    ...
    def referencingIdentifiers: overflowdb.traversal.Traversal[Identifier] =
        get().referencingIdentifiers
...
class MethodParameterInDb(ref: NodeRef[NodeDb]) extends NodeDb(ref) with StoredNode
    with AstNode with CfgNode with Declaration with MethodParameterInBase:
    ...
    override def propertiesMapForStorage: java.util.Map[String, Any] =
        ...
        if !(("<empty>") == evaluationStrategy) then
            properties.put("EVALUATION_STRATEGY", evaluationStrategy)
        if !((-1: Int) == index) then properties.put("INDEX", index)
        if !((false) == isVariadic) then properties.put("IS_VARIADIC", isVariadic)
        lineNumber.map { value => properties.put("LINE_NUMBER", value) }
    ...
    def typ: Type =
        try evalTypeOut.collectAll[Type].next()
        catch
    ...
    def parameterLinkOut: Iterator[MethodParameterOut] =
        createAdjacentNodeScalaIteratorByOffSet[MethodParameterOut](2)
    override def _parameterLinkOut = createAdjacentNodeScalaIteratorByOffSet[StoredNode](2)
    def asOutput: overflowdb.traversal.Traversal[MethodParameterOut] =
        parameterLinkOut.collectAll[MethodParameterOut]
    ...
    def _closureBindingViaRefIn: overflowdb.traversal.Traversal[ClosureBinding] =
        refIn.collectAll[ClosureBinding]
    def referencingIdentifiers: overflowdb.traversal.Traversal[Identifier] =
        refIn.collectAll[Identifier]
    ...
    override def canEqual(that: Any): Boolean =
        that != null && that.isInstanceOf[MethodParameterInDb]
    ...
    override def fromNewNode(newNode: NewNode, mapping: NewNode => StoredNode): Unit =
        ...
        this._dynamicTypeHintFullName =
            if newNode.asInstanceOf[NewMethodParameterIn].dynamicTypeHintFullName != null then
        ...
        this._possibleTypes =
            if newNode.asInstanceOf[NewMethodParameterIn].possibleTypes != null then
