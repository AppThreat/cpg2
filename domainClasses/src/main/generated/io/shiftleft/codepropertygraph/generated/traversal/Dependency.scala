...
class DependencyTraversalExtGen[NodeType <: Dependency](val traversal: Iterator[NodeType])
    extends AnyVal:
    ...
    def dependencyGroupId(pattern: String): Iterator[NodeType] =
        if !Misc.isRegex(pattern) then
            traversal.filter { node =>
                node.dependencyGroupId.isDefined && node.dependencyGroupId.get == pattern
            }
        else
            overflowdb.traversal.filter.StringPropertyFilter.regexp(
              traversal.filter(_.dependencyGroupId.isDefined)
            )(_.dependencyGroupId.get, pattern)

    /** Traverse to nodes where the dependencyGroupId matches at least one of the regular
    ...
    def dependencyGroupId(patterns: String*): Iterator[NodeType] =
        overflowdb.traversal.filter.StringPropertyFilter.regexpMultiple(
          traversal.filter(_.dependencyGroupId.isDefined)
        )(_.dependencyGroupId.get, patterns)
    ...
    def dependencyGroupIdNot(pattern: String): Iterator[NodeType] =
        if !Misc.isRegex(pattern) then
            traversal.filter { node =>
                node.dependencyGroupId.isEmpty || node.dependencyGroupId.get != pattern
            }
        else
    ...
    def versionExact(value: String): Iterator[NodeType] =
        val fastResult = traversal match
            case init: overflowdb.traversal.InitialTraversal[NodeType] =>
                init.getByIndex("VERSION", value).getOrElse(null)
            case _ => null
        if fastResult != null then fastResult
