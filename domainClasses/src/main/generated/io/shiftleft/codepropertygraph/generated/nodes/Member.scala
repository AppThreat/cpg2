...
class MemberDb(ref: NodeRef[NodeDb]) extends NodeDb(ref) with StoredNode
    with AstNode with Declaration with MemberBase:
    ...
    def typeDecl: TypeDecl =
        try astIn.collectAll[TypeDecl].next()
        catch
    ...
    override def fromNewNode(newNode: NewNode, mapping: NewNode => StoredNode): Unit =
        ...
        this._dynamicTypeHintFullName =
            if newNode.asInstanceOf[NewMember].dynamicTypeHintFullName != null then
        ...
        this._possibleTypes = if newNode.asInstanceOf[NewMember].possibleTypes != null then
            newNode.asInstanceOf[NewMember].possibleTypes
        else collection.immutable.ArraySeq.empty
