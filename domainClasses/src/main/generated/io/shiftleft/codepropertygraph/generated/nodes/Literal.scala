...
class Literal(graph_4762: Graph, id_4762: Long /*cf https://github.com/scala/bug/issues/4762 */ )
    ...
    /** Traverse to ARRAY_INITIALIZER via AST IN edge.
      */
    def _arrayInitializerViaAstIn: overflowdb.traversal.Traversal[ArrayInitializer] =
        get()._arrayInitializerViaAstIn
...
class LiteralDb(ref: NodeRef[NodeDb]) extends NodeDb(ref) with StoredNode
    ...
    def _arrayInitializerViaAstIn: overflowdb.traversal.Traversal[ArrayInitializer] =
        astIn.collectAll[ArrayInitializer]
    ...
    override def fromNewNode(newNode: NewNode, mapping: NewNode => StoredNode): Unit =
        ...
        this._dynamicTypeHintFullName =
            if newNode.asInstanceOf[NewLiteral].dynamicTypeHintFullName != null then
        ...
        this._possibleTypes = if newNode.asInstanceOf[NewLiteral].possibleTypes != null then
            newNode.asInstanceOf[NewLiteral].possibleTypes
        else collection.immutable.ArraySeq.empty
