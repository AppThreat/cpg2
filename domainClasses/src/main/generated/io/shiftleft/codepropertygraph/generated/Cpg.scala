...
object Cpg:
    ...
    /** Instantiate Cpg with storage. If the storage file already exists, it will load (a subset of)
      * the data into memory. Otherwise it will create an empty Cpg. In either case, configuring
      * storage means that OverflowDb will be stored to disk on shutdown (`close`). I.e. if you want
      * to preserve state between sessions, just use this method to instantiate the Cpg and ensure
      * to properly `close` it at the end.
      * @param path
    ...
    def convertPropertyForPersistence(property: Any): Any =
        property match
            case arraySeq: scala.collection.immutable.ArraySeq[?] => arraySeq.unsafeArray
...
class GeneratedNodeStarterExt(val wrapper: Cpg) extends AnyVal:
    ...
    @overflowdb.traversal.help.Doc(info =
        "All nodes of type Annotation, i.e. with label ANNOTATION"
    )
    ...
    @overflowdb.traversal.help.Doc(info =
        "All nodes of type AnnotationLiteral, i.e. with label ANNOTATION_LITERAL"
    )
    def annotationLiteral: Iterator[nodes.AnnotationLiteral] =
        overflowdb.traversal.InitialTraversal.from[nodes.AnnotationLiteral](
          wrapper.graph,
          "ANNOTATION_LITERAL"
        )

    @overflowdb.traversal.help.Doc(info =
        "All nodes of type AnnotationParameter, i.e. with label ANNOTATION_PARAMETER"
    )
    def annotationParameter: Iterator[nodes.AnnotationParameter] =
        overflowdb.traversal.InitialTraversal.from[nodes.AnnotationParameter](
          wrapper.graph,
    ...
    def annotationParameterAssign: Iterator[nodes.AnnotationParameterAssign] =
        overflowdb.traversal.InitialTraversal.from[nodes.AnnotationParameterAssign](
          wrapper.graph,
          "ANNOTATION_PARAMETER_ASSIGN"
        )

    @overflowdb.traversal.help.Doc(info =
        "All nodes of type ArrayInitializer, i.e. with label ARRAY_INITIALIZER"
    )
    def arrayInitializer: Iterator[nodes.ArrayInitializer] =
        overflowdb.traversal.InitialTraversal.from[nodes.ArrayInitializer](
          wrapper.graph,
          "ARRAY_INITIALIZER"
        )
    ...
    def block: Iterator[nodes.Block] =
        overflowdb.traversal.InitialTraversal.from[nodes.Block](wrapper.graph, "BLOCK")

    @overflowdb.traversal.help.Doc(info = "All nodes of type Call, i.e. with label CALL")
    def call: Iterator[nodes.Call] =
        overflowdb.traversal.InitialTraversal.from[nodes.Call](wrapper.graph, "CALL")

    @overflowdb.traversal.help.Doc(info =
        "All nodes of type ClosureBinding, i.e. with label CLOSURE_BINDING"
    )
    def closureBinding: Iterator[nodes.ClosureBinding] =
        overflowdb.traversal.InitialTraversal.from[nodes.ClosureBinding](
          wrapper.graph,
          "CLOSURE_BINDING"
        )
    ...
    @overflowdb.traversal.help.Doc(info =
        "All nodes of type ConfigFile, i.e. with label CONFIG_FILE"
    )
    ...
    @overflowdb.traversal.help.Doc(info =
        "All nodes of type ControlStructure, i.e. with label CONTROL_STRUCTURE"
    )
    def controlStructure: Iterator[nodes.ControlStructure] =
        overflowdb.traversal.InitialTraversal.from[nodes.ControlStructure](
          wrapper.graph,
          "CONTROL_STRUCTURE"
        )

    @overflowdb.traversal.help.Doc(info =
        "All nodes of type Dependency, i.e. with label DEPENDENCY"
    )
    ...
    @overflowdb.traversal.help.Doc(info =
        "All nodes of type FieldIdentifier, i.e. with label FIELD_IDENTIFIER"
    )
    def fieldIdentifier: Iterator[nodes.FieldIdentifier] =
        overflowdb.traversal.InitialTraversal.from[nodes.FieldIdentifier](
          wrapper.graph,
          "FIELD_IDENTIFIER"
        )

    @overflowdb.traversal.help.Doc(info = "All nodes of type File, i.e. with label FILE")
    def file: Iterator[nodes.File] =
        overflowdb.traversal.InitialTraversal.from[nodes.File](wrapper.graph, "FILE")
    ...
    @overflowdb.traversal.help.Doc(info =
        "All nodes of type Identifier, i.e. with label IDENTIFIER"
    )
    ...
    @overflowdb.traversal.help.Doc(info =
        "All nodes of type JumpTarget, i.e. with label JUMP_TARGET"
    )
    ...
    @overflowdb.traversal.help.Doc(info =
        "All nodes of type KeyValuePair, i.e. with label KEY_VALUE_PAIR"
    )
    def keyValuePair: Iterator[nodes.KeyValuePair] =
        overflowdb.traversal.InitialTraversal.from[nodes.KeyValuePair](
          wrapper.graph,
          "KEY_VALUE_PAIR"
        )
    ...
    def local: Iterator[nodes.Local] =
        overflowdb.traversal.InitialTraversal.from[nodes.Local](wrapper.graph, "LOCAL")
    ...
    def member: Iterator[nodes.Member] =
        overflowdb.traversal.InitialTraversal.from[nodes.Member](wrapper.graph, "MEMBER")
    ...
    def method: Iterator[nodes.Method] =
        overflowdb.traversal.InitialTraversal.from[nodes.Method](wrapper.graph, "METHOD")

    @overflowdb.traversal.help.Doc(info =
        "All nodes of type MethodParameterIn, i.e. with label METHOD_PARAMETER_IN"
    )
    def methodParameterIn: Iterator[nodes.MethodParameterIn] =
        overflowdb.traversal.InitialTraversal.from[nodes.MethodParameterIn](
          wrapper.graph,
          "METHOD_PARAMETER_IN"
        )

    @overflowdb.traversal.help.Doc(info =
        "All nodes of type MethodParameterOut, i.e. with label METHOD_PARAMETER_OUT"
    )
    def methodParameterOut: Iterator[nodes.MethodParameterOut] =
        overflowdb.traversal.InitialTraversal.from[nodes.MethodParameterOut](
          wrapper.graph,
    ...
    @overflowdb.traversal.help.Doc(info =
        "All nodes of type MethodReturn, i.e. with label METHOD_RETURN"
    )
    def methodReturn: Iterator[nodes.MethodReturn] =
        overflowdb.traversal.InitialTraversal.from[nodes.MethodReturn](
          wrapper.graph,
          "METHOD_RETURN"
        )
    ...
    @overflowdb.traversal.help.Doc(info =
        "All nodes of type NamespaceBlock, i.e. with label NAMESPACE_BLOCK"
    )
    def namespaceBlock: Iterator[nodes.NamespaceBlock] =
        overflowdb.traversal.InitialTraversal.from[nodes.NamespaceBlock](
          wrapper.graph,
          "NAMESPACE_BLOCK"
        )

    @overflowdb.traversal.help.Doc(info = "All nodes of type Return, i.e. with label RETURN")
    def ret: Iterator[nodes.Return] =
        overflowdb.traversal.InitialTraversal.from[nodes.Return](wrapper.graph, "RETURN")

    @overflowdb.traversal.help.Doc(info = "All nodes of type Tag, i.e. with label TAG")
    def tag: Iterator[nodes.Tag] =
        overflowdb.traversal.InitialTraversal.from[nodes.Tag](wrapper.graph, "TAG")

    @overflowdb.traversal.help.Doc(info =
        "All nodes of type TagNodePair, i.e. with label TAG_NODE_PAIR"
    )
    def tagNodePair: Iterator[nodes.TagNodePair] =
        overflowdb.traversal.InitialTraversal.from[nodes.TagNodePair](
          wrapper.graph,
          "TAG_NODE_PAIR"
        )

    @overflowdb.traversal.help.Doc(info =
        "All nodes of type TemplateDom, i.e. with label TEMPLATE_DOM"
    )
    ...
    def typ: Iterator[nodes.Type] =
        overflowdb.traversal.InitialTraversal.from[nodes.Type](wrapper.graph, "TYPE")

    @overflowdb.traversal.help.Doc(info =
        "All nodes of type TypeArgument, i.e. with label TYPE_ARGUMENT"
    )
    def typeArgument: Iterator[nodes.TypeArgument] =
        overflowdb.traversal.InitialTraversal.from[nodes.TypeArgument](
          wrapper.graph,
          "TYPE_ARGUMENT"
        )
    ...
    @overflowdb.traversal.help.Doc(info =
        "All nodes of type TypeParameter, i.e. with label TYPE_PARAMETER"
    )
    def typeParameter: Iterator[nodes.TypeParameter] =
        overflowdb.traversal.InitialTraversal.from[nodes.TypeParameter](
          wrapper.graph,
          "TYPE_PARAMETER"
        )
    ...
    def astNode: Iterator[nodes.AstNode] = wrapper.graph.nodes(
      "ANNOTATION",
      "ANNOTATION_LITERAL",
      "ANNOTATION_PARAMETER",
      "ANNOTATION_PARAMETER_ASSIGN",
      ...
      "TYPE_REF",
      "UNKNOWN"
    ).asScala.asInstanceOf[Iterator[nodes.AstNode]]

    @overflowdb.traversal.help.Doc(info = "All nodes of type CallRepr, i.e. with label in CALL")
    def callRepr: Iterator[nodes.CallRepr] =
        wrapper.graph.nodes("CALL").asScala.asInstanceOf[Iterator[nodes.CallRepr]]
    ...
    def cfgNode: Iterator[nodes.CfgNode] = wrapper.graph.nodes(
      "ANNOTATION",
      "ANNOTATION_LITERAL",
      "ARRAY_INITIALIZER",
      "BLOCK",
      ...
      "TYPE_REF",
      "UNKNOWN"
    ).asScala.asInstanceOf[Iterator[nodes.CfgNode]]

    @overflowdb.traversal.help.Doc(info =
        "All nodes of type Declaration, i.e. with label in LOCAL, MEMBER, METHOD, METHOD_PARAMETER_IN, METHOD_PARAMETER_OUT"
    )
    def declaration: Iterator[nodes.Declaration] = wrapper.graph.nodes(
      "LOCAL",
      "MEMBER",
      "METHOD",
      "METHOD_PARAMETER_IN",
      "METHOD_PARAMETER_OUT"
    ).asScala.asInstanceOf[Iterator[nodes.Declaration]]
    ...
    def expression: Iterator[nodes.Expression] = wrapper.graph.nodes(
      "ANNOTATION",
      "ANNOTATION_LITERAL",
      "ARRAY_INITIALIZER",
      "BLOCK",
      ...
      "TYPE_REF",
      "UNKNOWN"
    ).asScala.asInstanceOf[Iterator[nodes.Expression]]
end GeneratedNodeStarterExt

/** Domain-specific version of diffgraph builder. This is to allow schema checking before diffgraph
  * application in the future, as well as a schema-aware point for providing backwards compatibility
  * in odbv2.
  */
class DiffGraphBuilder extends overflowdb.BatchedUpdate.DiffGraphBuilder:
    override def absorb(other: overflowdb.BatchedUpdate.DiffGraphBuilder): this.type =
        super.absorb(other); this
    override def addNode(node: overflowdb.DetachedNodeData): this.type =
        super.addNode(node); this
    override def addNode(label: String, keyvalues: Any*): this.type =
        super.addNode(label, keyvalues*); this
    override def addEdge(
      src: overflowdb.NodeOrDetachedNode,
      dst: overflowdb.NodeOrDetachedNode,
      label: String
    ): this.type =
        super.addEdge(src, dst, label); this
    override def addEdge(
      src: overflowdb.NodeOrDetachedNode,
      dst: overflowdb.NodeOrDetachedNode,
      label: String,
      properties: Any*
        ...
        super.addEdge(src, dst, label, properties*); this
    ...
    override def removeNode(node: overflowdb.Node): this.type =
        super.removeNode(node); this
    override def removeEdge(edge: overflowdb.Edge): this.type =
        super.removeEdge(edge); this
end DiffGraphBuilder
