...
object FieldIdentifier:
    ...
    object PropertyNames:
        ...
        val Order         = "ORDER"
        val all: Set[String] =
            Set(ArgumentIndex, ArgumentName, CanonicalName, Code, ColumnNumber, LineNumber, Order)
    ...
    object Edges:
        val Out: Array[String] =
            Array("ARGUMENT", "CDG", "CFG", "DOMINATE", "POST_DOMINATE", "TAGGED_BY")
...
class FieldIdentifierDb(ref: NodeRef[NodeDb]) extends NodeDb(ref) with StoredNode
    ...
    override def propertiesMapForStorage: java.util.Map[String, Any] =
        ...
        argumentName.map { value => properties.put("ARGUMENT_NAME", value) }
        if !(("<empty>") == canonicalName) then properties.put("CANONICAL_NAME", canonicalName)
    ...
    def _callViaArgumentIn: Call =
        try argumentIn.collectAll[Call].next()
        catch
    ...
    def _callViaAstIn: Call =
        try astIn.collectAll[Call].next()
        catch
            case e: java.util.NoSuchElementException =>
                throw new overflowdb.SchemaViolationException(
    ...
    def reachingDefIn: Iterator[TemplateDom] =
        createAdjacentNodeScalaIteratorByOffSet[TemplateDom](13)
