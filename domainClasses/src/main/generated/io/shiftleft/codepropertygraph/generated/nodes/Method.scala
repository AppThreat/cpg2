...
object Method:
    ...
    object Edges:
        val Out: Array[String] =
            Array("AST", "CFG", "CONTAINS", "DOMINATE", "REACHING_DEF", "SOURCE_FILE", "TAGGED_BY")
...
class Method(graph_4762: Graph, id_4762: Long /*cf https://github.com/scala/bug/issues/4762 */ )
    ...
    with MethodBase
    with StoredNode
    with AstNode with CfgNode with Declaration:
    ...
    /** Traverse to TYPE_PARAMETER via AST OUT edge.
      */
    def _typeParameterViaAstOut: overflowdb.traversal.Traversal[TypeParameter] =
        get()._typeParameterViaAstOut
    ...
    /** Traverse to IDENTIFIER via CONTAINS OUT edge.
      */
    def _identifierViaContainsOut: overflowdb.traversal.Traversal[Identifier] =
        get()._identifierViaContainsOut

    /** Traverse to JUMP_TARGET via CONTAINS OUT edge.
      */
    def _jumpTargetViaContainsOut: overflowdb.traversal.Traversal[JumpTarget] =
        get()._jumpTargetViaContainsOut
    ...
    /** Traverse to METHOD_REF via CONTAINS OUT edge.
      */
    def _methodRefViaContainsOut: overflowdb.traversal.Traversal[MethodRef] =
        get()._methodRefViaContainsOut
    ...
    /** Traverse to TEMPLATE_DOM via CONTAINS OUT edge.
      */
    def _templateDomViaContainsOut: overflowdb.traversal.Traversal[TemplateDom] =
        get()._templateDomViaContainsOut

    /** Traverse to TYPE_REF via CONTAINS OUT edge.
      */
    def _typeRefViaContainsOut: overflowdb.traversal.Traversal[TypeRef] =
        get()._typeRefViaContainsOut

    /** Traverse to UNKNOWN via CONTAINS OUT edge.
      */
    def _unknownViaContainsOut: overflowdb.traversal.Traversal[Unknown] =
        get()._unknownViaContainsOut
    ...
    /** Traverse to TYPE_DECL via CONTAINS IN edge.
      */
    def _typeDeclViaContainsIn: overflowdb.traversal.Traversal[TypeDecl] =
        get()._typeDeclViaContainsIn
...
class MethodDb(ref: NodeRef[NodeDb]) extends NodeDb(ref) with StoredNode
    with AstNode with CfgNode with Declaration with MethodBase:
    ...
    override def propertiesMapForStorage: java.util.Map[String, Any] =
        ...
        if !(("<empty>") == astParentFullName) then
            properties.put("AST_PARENT_FULL_NAME", astParentFullName)
        if !(("<empty>") == astParentType) then properties.put("AST_PARENT_TYPE", astParentType)
        if !(("<empty>") == code) then properties.put("CODE", code)
        columnNumber.map { value => properties.put("COLUMN_NUMBER", value) }
        columnNumberEnd.map { value => properties.put("COLUMN_NUMBER_END", value) }
        if !(("<empty>") == filename) then properties.put("FILENAME", filename)
        if !(("<empty>") == fullName) then properties.put("FULL_NAME", fullName)
        hash.map { value => properties.put("HASH", value) }
        if !((false) == isExternal) then properties.put("IS_EXTERNAL", isExternal)
        lineNumber.map { value => properties.put("LINE_NUMBER", value) }
        lineNumberEnd.map { value => properties.put("LINE_NUMBER_END", value) }
    ...
    def block: Block =
        try astOut.collectAll[Block].next()
        catch
    ...
    def parameter: overflowdb.traversal.Traversal[MethodParameterIn] =
        astOut.collectAll[MethodParameterIn]
    def _methodParameterOutViaAstOut: overflowdb.traversal.Traversal[MethodParameterOut] =
        astOut.collectAll[MethodParameterOut]
    def methodReturn: MethodReturn =
        try astOut.collectAll[MethodReturn].next()
        catch
            case e: java.util.NoSuchElementException =>
                throw new overflowdb.SchemaViolationException(
    ...
    def _typeParameterViaAstOut: overflowdb.traversal.Traversal[TypeParameter] =
        astOut.collectAll[TypeParameter]
    ...
    def _identifierViaContainsOut: overflowdb.traversal.Traversal[Identifier] =
        containsOut.collectAll[Identifier]
    def _jumpTargetViaContainsOut: overflowdb.traversal.Traversal[JumpTarget] =
        containsOut.collectAll[JumpTarget]
    def literal: overflowdb.traversal.Traversal[Literal] = containsOut.collectAll[Literal]
    def _methodRefViaContainsOut: overflowdb.traversal.Traversal[MethodRef] =
        containsOut.collectAll[MethodRef]
    def _returnViaContainsOut: overflowdb.traversal.Traversal[Return] =
        containsOut.collectAll[Return]
    def _templateDomViaContainsOut: overflowdb.traversal.Traversal[TemplateDom] =
        containsOut.collectAll[TemplateDom]
    def _typeRefViaContainsOut: overflowdb.traversal.Traversal[TypeRef] =
        containsOut.collectAll[TypeRef]
    def _unknownViaContainsOut: overflowdb.traversal.Traversal[Unknown] =
        containsOut.collectAll[Unknown]
    ...
    def _namespaceBlockViaAstIn: Option[NamespaceBlock] =
        astIn.collectAll[NamespaceBlock].nextOption()
    ...
    def _typeDeclViaContainsIn: overflowdb.traversal.Traversal[TypeDecl] =
        containsIn.collectAll[TypeDecl]
    ...
    override def fromNewNode(newNode: NewNode, mapping: NewNode => StoredNode): Unit =
        ...
        graph.indexManager.putIfIndexed(
          "FULL_NAME",
          newNode.asInstanceOf[NewMethod].fullName,
          this.ref
        )
    end fromNewNode
end MethodDb
