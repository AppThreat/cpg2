...
class FindingDb(ref: NodeRef[NodeDb]) extends NodeDb(ref) with StoredNode
    ...
    override def propertiesMap: java.util.Map[String, Any] =
        ...
        if this._evidence != null && this._evidence.nonEmpty then
            properties.put("evidence", this.evidence)
        if this._keyValuePairs != null && this._keyValuePairs.nonEmpty then
    ...
    override protected def updateSpecificProperty(key: String, value: Object): Unit =
        key match
            ...
            case "evidence" => this._evidence = value match
                        ...
                        if jCollection.iterator.hasNext then
                            collection.immutable.ArraySeq.unsafeWrapArray(
                              jCollection.asInstanceOf[
                                java.util.Collection[StoredNode]
                    ...
                    case iter: Iterable[?] =>
                        if iter.nonEmpty then
                            collection.immutable.ArraySeq.unsafeWrapArray(
                              iter.asInstanceOf[Iterable[StoredNode]].toArray
                            )
                        else collection.immutable.ArraySeq.empty
            case "keyValuePairs" => this._keyValuePairs = value match
                    ...
                    case arr: Array[?] => collection.immutable.ArraySeq.unsafeWrapArray(
                          arr
                        ).asInstanceOf[IndexedSeq[KeyValuePair]]
                    case jCollection: java.lang.Iterable[?] =>
                        if jCollection.iterator.hasNext then
                            collection.immutable.ArraySeq.unsafeWrapArray(
                              jCollection.asInstanceOf[
                    ...
                    case iter: Iterable[?] =>
                        if iter.nonEmpty then
                            collection.immutable.ArraySeq.unsafeWrapArray(
                              iter.asInstanceOf[Iterable[KeyValuePair]].toArray
                            )
                        else collection.immutable.ArraySeq.empty
    ...
    override def fromNewNode(newNode: NewNode, mapping: NewNode => StoredNode): Unit =
        ...
        this._evidence =
            if newNode.asInstanceOf[NewFinding].evidence == null || newNode.asInstanceOf[
                  NewFinding
                ].evidence.isEmpty
            then
                collection.immutable.ArraySeq.empty
                ...
                collection.immutable.ArraySeq.unsafeWrapArray(
                  newNode.asInstanceOf[NewFinding].evidence.map {
                      case null => throw new NullPointerException(
                            "NullPointers forbidden in contained nodes"
                          )
        ...
        this._keyValuePairs =
            if newNode.asInstanceOf[NewFinding].keyValuePairs == null || newNode.asInstanceOf[
                  NewFinding
            ...
            then
                ...
                collection.immutable.ArraySeq.unsafeWrapArray(
                  newNode.asInstanceOf[NewFinding].keyValuePairs.map {
                      case null => throw new NullPointerException(
                            "NullPointers forbidden in contained nodes"
                          )
