...
class ConfigFileTraversalExtGen[NodeType <: ConfigFile](val traversal: Iterator[NodeType])
    extends AnyVal:
    ...
    /** Traverse to nodes where the content matches at least one of the regular expressions in
      * `values`
      */
    ...
    def contentExact(value: String): Iterator[NodeType] =
        val fastResult = traversal match
            case init: overflowdb.traversal.InitialTraversal[NodeType] =>
                init.getByIndex("CONTENT", value).getOrElse(null)
            case _ => null
