...
class Local(graph_4762: Graph, id_4762: Long /*cf https://github.com/scala/bug/issues/4762 */ )
    ...
    /** Places (identifier) where this local is being referenced Traverse to IDENTIFIER via REF IN
      * edge.
      */
...
class LocalDb(ref: NodeRef[NodeDb]) extends NodeDb(ref) with StoredNode
    with AstNode with Declaration with LocalBase:
    ...
    def capturedByOut: Iterator[ClosureBinding] =
        createAdjacentNodeScalaIteratorByOffSet[ClosureBinding](0)
    ...
    override def fromNewNode(newNode: NewNode, mapping: NewNode => StoredNode): Unit =
        ...
        this._dynamicTypeHintFullName =
            if newNode.asInstanceOf[NewLocal].dynamicTypeHintFullName != null then
        ...
        this._possibleTypes = if newNode.asInstanceOf[NewLocal].possibleTypes != null then
            newNode.asInstanceOf[NewLocal].possibleTypes
        else collection.immutable.ArraySeq.empty
