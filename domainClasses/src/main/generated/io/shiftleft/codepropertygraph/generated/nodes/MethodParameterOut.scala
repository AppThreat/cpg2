...
end MethodParameterOut

trait MethodParameterOutBase extends AbstractNode with AstNodeBase with CfgNodeBase
...
class MethodParameterOutDb(ref: NodeRef[NodeDb]) extends NodeDb(ref) with StoredNode
    with AstNode with CfgNode with Declaration with MethodParameterOutBase:
    ...
    def reachingDefOut: Iterator[Expression] =
        createAdjacentNodeScalaIteratorByOffSet[Expression](1)
    ...
    def parameterLinkIn: Iterator[MethodParameterIn] =
        createAdjacentNodeScalaIteratorByOffSet[MethodParameterIn](5)
    override def _parameterLinkIn = createAdjacentNodeScalaIteratorByOffSet[StoredNode](5)
    def asInput: overflowdb.traversal.Traversal[MethodParameterIn] =
        parameterLinkIn.collectAll[MethodParameterIn]
    ...
    override def canEqual(that: Any): Boolean =
        that != null && that.isInstanceOf[MethodParameterOutDb]
