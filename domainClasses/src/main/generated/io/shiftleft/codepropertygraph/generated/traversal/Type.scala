...
class TypeTraversalExtGen[NodeType <: Type](val traversal: Iterator[NodeType]) extends AnyVal:
    ...
    /** Traverse to nodes where the typeDeclFullName matches at least one of the regular expressions
      * in `values`
      */
    ...
    def typeDeclFullNameExact(values: String*): Iterator[NodeType] =
        if values.size == 1 then
            ...
            overflowdb.traversal.filter.StringPropertyFilter.exactMultiple[NodeType, String](
              traversal,
              node => Some(node.typeDeclFullName),
              values,
              "TYPE_DECL_FULL_NAME"
            )
    ...
    /** Traverse to nodes where typeDeclFullName does not match any of the regular expressions in
      * `values`.
      */
