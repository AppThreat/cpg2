...
class LocationTraversalExtGen[NodeType <: Location](val traversal: Iterator[NodeType])
    extends AnyVal:
    ...
    def classNameExact(value: String): Iterator[NodeType] =
        val fastResult = traversal match
            case init: overflowdb.traversal.InitialTraversal[NodeType] =>
                init.getByIndex("CLASS_NAME", value).getOrElse(null)
            case _ => null
    ...
    /** Traverse to nodes where the classShortName matches at least one of the regular expressions
      * in `values`
      */
    ...
    /** Traverse to nodes where the methodFullName matches at least one of the regular expressions
      * in `values`
      */
    ...
    /** Traverse to nodes where the methodShortName matches at least one of the regular expressions
      * in `values`
      */
    ...
    def methodShortNameExact(values: String*): Iterator[NodeType] =
        if values.size == 1 then
            ...
            overflowdb.traversal.filter.StringPropertyFilter.exactMultiple[NodeType, String](
              traversal,
              node => Some(node.methodShortName),
              values,
              "METHOD_SHORT_NAME"
            )
    ...
    def nodeLabelExact(value: String): Iterator[NodeType] =
        val fastResult = traversal match
            case init: overflowdb.traversal.InitialTraversal[NodeType] =>
                init.getByIndex("NODE_LABEL", value).getOrElse(null)
            case _ => null
        if fastResult != null then fastResult
    ...
    def symbolExact(value: String): Iterator[NodeType] =
        val fastResult = traversal match
            case init: overflowdb.traversal.InitialTraversal[NodeType] =>
                init.getByIndex("SYMBOL", value).getOrElse(null)
            case _ => null
        if fastResult != null then fastResult
