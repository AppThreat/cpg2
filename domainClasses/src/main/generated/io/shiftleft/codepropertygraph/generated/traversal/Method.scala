...
class MethodTraversalExtGen[NodeType <: Method](val traversal: Iterator[NodeType]) extends AnyVal:
    ...
    def columnNumberEnd(value: Integer): Iterator[NodeType] =
        traversal.filter { node =>
            node.columnNumberEnd.isDefined && node.columnNumberEnd.get == value
        }
    ...
    def columnNumberEnd(values: Integer*): Iterator[NodeType] =
        val vset = values.toSet
        traversal.filter { node =>
            node.columnNumberEnd.isDefined && vset.contains(node.columnNumberEnd.get)
        }
    ...
    def hash(pattern: String): Iterator[NodeType] =
        if !Misc.isRegex(pattern) then
            ...
            overflowdb.traversal.filter.StringPropertyFilter.regexp(
              traversal.filter(_.hash.isDefined)
            )(_.hash.get, pattern)
    ...
    def lineNumberEnd(values: Integer*): Iterator[NodeType] =
        val vset = values.toSet
        traversal.filter { node =>
            node.lineNumberEnd.isDefined && vset.contains(node.lineNumberEnd.get)
        }
    ...
    def lineNumberEndNot(value: Integer): Iterator[NodeType] =
        traversal.filter { node =>
            !node.lineNumberEnd.isDefined || node.lineNumberEnd.get != value
        }
