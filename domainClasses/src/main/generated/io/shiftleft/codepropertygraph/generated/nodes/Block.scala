...
class Block(graph_4762: Graph, id_4762: Long /*cf https://github.com/scala/bug/issues/4762 */ )
    ...
    /** Traverse to BLOCK via REACHING_DEF OUT edge.
      */
    def _blockViaReachingDefOut: overflowdb.traversal.Traversal[Block] =
        get()._blockViaReachingDefOut
...
class BlockDb(ref: NodeRef[NodeDb]) extends NodeDb(ref) with StoredNode
    ...
    def _blockViaReachingDefOut: overflowdb.traversal.Traversal[Block] =
        reachingDefOut.collectAll[Block]
    ...
    def _blockViaAstIn: Block =
        try astIn.collectAll[Block].next()
        catch
    ...
    def _methodViaAstIn: Method =
        try astIn.collectAll[Method].next()
        catch
            case e: java.util.NoSuchElementException =>
                throw new overflowdb.SchemaViolationException(
    ...
    def reachingDefIn: Iterator[Expression] =
        createAdjacentNodeScalaIteratorByOffSet[Expression](17)
    ...
    override def fromNewNode(newNode: NewNode, mapping: NewNode => StoredNode): Unit =
        ...
        this._dynamicTypeHintFullName =
            if newNode.asInstanceOf[NewBlock].dynamicTypeHintFullName != null then
        ...
        this._possibleTypes = if newNode.asInstanceOf[NewBlock].possibleTypes != null then
            newNode.asInstanceOf[NewBlock].possibleTypes
        else collection.immutable.ArraySeq.empty
