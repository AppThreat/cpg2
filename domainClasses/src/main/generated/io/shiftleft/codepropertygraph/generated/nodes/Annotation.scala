...
object Annotation:
    ...
    object PropertyNames:
        ...
        val Order         = "ORDER"
        val all: Set[String] =
            Set(ArgumentIndex, ArgumentName, Code, ColumnNumber, FullName, LineNumber, Name, Order)
...
class Annotation(graph_4762: Graph, id_4762: Long /*cf https://github.com/scala/bug/issues/4762 */ )
    ...
    /** Traverse to ANNOTATION_PARAMETER_ASSIGN via AST OUT edge.
      */
    def _annotationParameterAssignViaAstOut
      : overflowdb.traversal.Traversal[AnnotationParameterAssign] =
    ...
    /** Traverse to ANNOTATION_PARAMETER_ASSIGN via AST IN edge.
      */
    def _annotationParameterAssignViaAstIn
      : overflowdb.traversal.Traversal[AnnotationParameterAssign] =
...
class AnnotationDb(ref: NodeRef[NodeDb]) extends NodeDb(ref) with StoredNode
    with Expression with AnnotationBase:
    ...
    def _identifierViaAstIn: overflowdb.traversal.Traversal[Identifier] =
        astIn.collectAll[Identifier]
    ...
    def reachingDefIn: Iterator[TemplateDom] =
        createAdjacentNodeScalaIteratorByOffSet[TemplateDom](4)
    ...
    override def fromNewNode(newNode: NewNode, mapping: NewNode => StoredNode): Unit =
        ...
        graph.indexManager.putIfIndexed(
          "FULL_NAME",
          newNode.asInstanceOf[NewAnnotation].fullName,
          this.ref
        )
end AnnotationDb
