...
class MetaDataTraversalExtGen[NodeType <: MetaData](val traversal: Iterator[NodeType])
    extends AnyVal:
    ...
    def languageExact(value: String): Iterator[NodeType] =
        val fastResult = traversal match
            case init: overflowdb.traversal.InitialTraversal[NodeType] =>
                init.getByIndex("LANGUAGE", value).getOrElse(null)
            case _ => null
    ...
    def rootExact(value: String): Iterator[NodeType] =
        val fastResult = traversal match
            case init: overflowdb.traversal.InitialTraversal[NodeType] =>
                init.getByIndex("ROOT", value).getOrElse(null)
            case _ => null
        if fastResult != null then fastResult
