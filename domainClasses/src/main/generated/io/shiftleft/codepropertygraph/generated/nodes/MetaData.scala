...
class MetaDataDb(ref: NodeRef[NodeDb]) extends NodeDb(ref) with StoredNode
    ...
    override def propertiesMap: java.util.Map[String, Any] =
        ...
        if this._overlays != null && this._overlays.nonEmpty then
            properties.put("OVERLAYS", overlays)
    ...
    override def propertiesMapForStorage: java.util.Map[String, Any] =
        val properties = new java.util.HashMap[String, Any]
        hash.map { value => properties.put("HASH", value) }
        if !(("<empty>") == language) then properties.put("LANGUAGE", language)
        if this._overlays != null && this._overlays.nonEmpty then
            properties.put("OVERLAYS", overlays)
        if !(("<empty>") == root) then properties.put("ROOT", root)
        if !(("<empty>") == version) then properties.put("VERSION", version)
    ...
    override def fromNewNode(newNode: NewNode, mapping: NewNode => StoredNode): Unit =
        ...
        this._overlays = if newNode.asInstanceOf[NewMetaData].overlays != null then
            newNode.asInstanceOf[NewMetaData].overlays
        else collection.immutable.ArraySeq.empty
