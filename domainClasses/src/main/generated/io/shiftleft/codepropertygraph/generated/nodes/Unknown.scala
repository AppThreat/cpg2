...
class Unknown(graph_4762: Graph, id_4762: Long /*cf https://github.com/scala/bug/issues/4762 */ )
    ...
    def _methodParameterInViaAstIn: overflowdb.traversal.Traversal[MethodParameterIn] =
        get()._methodParameterInViaAstIn
...
class UnknownDb(ref: NodeRef[NodeDb]) extends NodeDb(ref) with StoredNode
    ...
    override def propertiesMapForStorage: java.util.Map[String, Any] =
        ...
        columnNumber.map { value => properties.put("COLUMN_NUMBER", value) }
        if !(("<empty>") == containedRef) then properties.put("CONTAINED_REF", containedRef)
        if this._dynamicTypeHintFullName != null && this._dynamicTypeHintFullName.nonEmpty then
            properties.put("DYNAMIC_TYPE_HINT_FULL_NAME", dynamicTypeHintFullName)
        lineNumber.map { value => properties.put("LINE_NUMBER", value) }
        if !((-1: Int) == order) then properties.put("ORDER", order)
        if !(("<empty>") == parserTypeName) then properties.put("PARSER_TYPE_NAME", parserTypeName)
        if this._possibleTypes != null && this._possibleTypes.nonEmpty then
            properties.put("POSSIBLE_TYPES", possibleTypes)
    ...
    def reachingDefIn: Iterator[TemplateDom] =
        createAdjacentNodeScalaIteratorByOffSet[TemplateDom](17)
    ...
    override def fromNewNode(newNode: NewNode, mapping: NewNode => StoredNode): Unit =
        ...
        this._dynamicTypeHintFullName =
            if newNode.asInstanceOf[NewUnknown].dynamicTypeHintFullName != null then
        ...
        this._possibleTypes = if newNode.asInstanceOf[NewUnknown].possibleTypes != null then
            newNode.asInstanceOf[NewUnknown].possibleTypes
        else collection.immutable.ArraySeq.empty
