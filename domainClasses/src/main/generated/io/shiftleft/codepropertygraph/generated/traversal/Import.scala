...
class ImportTraversalExtGen[NodeType <: Import](val traversal: Iterator[NodeType]) extends AnyVal:
    ...
    def importedAs(pattern: String): Iterator[NodeType] =
        if !Misc.isRegex(pattern) then
            ...
            overflowdb.traversal.filter.StringPropertyFilter.regexp(
              traversal.filter(_.importedAs.isDefined)
            )(_.importedAs.get, pattern)
    ...
    def importedEntity(pattern: String): Iterator[NodeType] =
        if !Misc.isRegex(pattern) then
            traversal.filter { node =>
                node.importedEntity.isDefined && node.importedEntity.get == pattern
            }
        else
            overflowdb.traversal.filter.StringPropertyFilter.regexp(
              traversal.filter(_.importedEntity.isDefined)
            )(_.importedEntity.get, pattern)

    /** Traverse to nodes where the importedEntity matches at least one of the regular expressions
      * in `values`
      */
    def importedEntity(patterns: String*): Iterator[NodeType] =
        overflowdb.traversal.filter.StringPropertyFilter.regexpMultiple(
          traversal.filter(_.importedEntity.isDefined)
        )(_.importedEntity.get, patterns)
    ...
    def importedEntityNot(pattern: String): Iterator[NodeType] =
        if !Misc.isRegex(pattern) then
            traversal.filter { node =>
                node.importedEntity.isEmpty || node.importedEntity.get != pattern
            }
        else
            overflowdb.traversal.filter.StringPropertyFilter.regexpNot(
              traversal.filter(_.importedEntity.isDefined)
