...
class ClosureBindingTraversalExtGen[NodeType <: ClosureBinding](val traversal: Iterator[NodeType])
    extends AnyVal:
    ...
    def closureBindingId(pattern: String): Iterator[NodeType] =
        if !Misc.isRegex(pattern) then
            traversal.filter { node =>
                node.closureBindingId.isDefined && node.closureBindingId.get == pattern
            }
        else
            overflowdb.traversal.filter.StringPropertyFilter.regexp(
              traversal.filter(_.closureBindingId.isDefined)
            )(_.closureBindingId.get, pattern)

    /** Traverse to nodes where the closureBindingId matches at least one of the regular expressions
      * in `values`
      */
    def closureBindingId(patterns: String*): Iterator[NodeType] =
        overflowdb.traversal.filter.StringPropertyFilter.regexpMultiple(
          traversal.filter(_.closureBindingId.isDefined)
        )(_.closureBindingId.get, patterns)
    ...
    def closureBindingIdNot(pattern: String): Iterator[NodeType] =
        if !Misc.isRegex(pattern) then
            traversal.filter { node =>
                node.closureBindingId.isEmpty || node.closureBindingId.get != pattern
            }
        else
    ...
    def closureOriginalName(pattern: String): Iterator[NodeType] =
        if !Misc.isRegex(pattern) then
            traversal.filter { node =>
                node.closureOriginalName.isDefined && node.closureOriginalName.get == pattern
            }
        else
            overflowdb.traversal.filter.StringPropertyFilter.regexp(
              traversal.filter(_.closureOriginalName.isDefined)
            )(_.closureOriginalName.get, pattern)

    /** Traverse to nodes where the closureOriginalName matches at least one of the regular
      * expressions in `values`
      */
    def closureOriginalName(patterns: String*): Iterator[NodeType] =
        overflowdb.traversal.filter.StringPropertyFilter.regexpMultiple(
          traversal.filter(_.closureOriginalName.isDefined)
        )(_.closureOriginalName.get, patterns)
    ...
    def closureOriginalNameNot(pattern: String): Iterator[NodeType] =
        if !Misc.isRegex(pattern) then
            traversal.filter { node =>
                node.closureOriginalName.isEmpty || node.closureOriginalName.get != pattern
            }
        else
            overflowdb.traversal.filter.StringPropertyFilter.regexpNot(
              traversal.filter(_.closureOriginalName.isDefined)
    ...
    /** Traverse to nodes where closureOriginalName does not match any of the regular expressions in
      * `values`.
      */
    ...
    /** Traverse to nodes where the evaluationStrategy matches at least one of the regular
      * expressions in `values`
      */
    ...
    /** Traverse to nodes where evaluationStrategy does not match any of the regular expressions in
      * `values`.
      */
...
end ClosureBindingTraversalExtGen
