...
class LocationDb(ref: NodeRef[NodeDb]) extends NodeDb(ref) with StoredNode
    ...
    override def propertiesMapForStorage: java.util.Map[String, Any] =
        val properties = new java.util.HashMap[String, Any]
        if !(("<empty>") == className) then properties.put("CLASS_NAME", className)
        if !(("<empty>") == classShortName) then properties.put("CLASS_SHORT_NAME", classShortName)
        ...
        if !(("<empty>") == methodShortName) then
            properties.put("METHOD_SHORT_NAME", methodShortName)
        if !(("<empty>") == nodeLabel) then properties.put("NODE_LABEL", nodeLabel)
        if !(("<empty>") == packageName) then properties.put("PACKAGE_NAME", packageName)
        if !(("<empty>") == symbol) then properties.put("SYMBOL", symbol)
        node.map { value => properties.put("node", value) }
        properties
