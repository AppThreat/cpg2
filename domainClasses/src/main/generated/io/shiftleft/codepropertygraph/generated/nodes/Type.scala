...
class Type(graph_4762: Graph, id_4762: Long /*cf https://github.com/scala/bug/issues/4762 */ )
    ...
    /** Traverse to TYPE_ARGUMENT via AST OUT edge.
      */
    def _typeArgumentViaAstOut: overflowdb.traversal.Traversal[TypeArgument] =
        get()._typeArgumentViaAstOut
    ...
    /** Traverse to IDENTIFIER via EVAL_TYPE IN edge.
      */
    def _identifierViaEvalTypeIn: overflowdb.traversal.Traversal[Identifier] =
        get()._identifierViaEvalTypeIn
    ...
    /** Traverse to METHOD_REF via EVAL_TYPE IN edge.
      */
    def _methodRefViaEvalTypeIn: overflowdb.traversal.Traversal[MethodRef] =
        get()._methodRefViaEvalTypeIn

    /** Traverse to METHOD_RETURN via EVAL_TYPE IN edge.
      */
    def _methodReturnViaEvalTypeIn: overflowdb.traversal.Traversal[MethodReturn] =
        get()._methodReturnViaEvalTypeIn
    ...
    /** Traverse to TYPE_DECL via INHERITS_FROM IN edge.
      */
    def _typeDeclViaInheritsFromIn: overflowdb.traversal.Traversal[TypeDecl] =
        get()._typeDeclViaInheritsFromIn
    ...
    /** Traverse to TYPE_ARGUMENT via REF IN edge.
      */
    def _typeArgumentViaRefIn: overflowdb.traversal.Traversal[TypeArgument] =
        get()._typeArgumentViaRefIn
...
class TypeDb(ref: NodeRef[NodeDb]) extends NodeDb(ref) with StoredNode
    ...
    override def propertiesMapForStorage: java.util.Map[String, Any] =
        ...
        if !(("<empty>") == typeDeclFullName) then
            properties.put("TYPE_DECL_FULL_NAME", typeDeclFullName)
    ...
    def _typeArgumentViaAstOut: overflowdb.traversal.Traversal[TypeArgument] =
        astOut.collectAll[TypeArgument]
    ...
    def _identifierViaEvalTypeIn: overflowdb.traversal.Traversal[Identifier] =
        evalTypeIn.collectAll[Identifier]
    def _literalViaEvalTypeIn: overflowdb.traversal.Traversal[Literal] =
        evalTypeIn.collectAll[Literal]
    ...
    def _methodRefViaEvalTypeIn: overflowdb.traversal.Traversal[MethodRef] =
        evalTypeIn.collectAll[MethodRef]
    def _methodReturnViaEvalTypeIn: overflowdb.traversal.Traversal[MethodReturn] =
        evalTypeIn.collectAll[MethodReturn]
    def _typeRefViaEvalTypeIn: overflowdb.traversal.Traversal[TypeRef] =
        evalTypeIn.collectAll[TypeRef]
    def _unknownViaEvalTypeIn: overflowdb.traversal.Traversal[Unknown] =
        evalTypeIn.collectAll[Unknown]
    ...
    def _typeDeclViaInheritsFromIn: overflowdb.traversal.Traversal[TypeDecl] =
        inheritsFromIn.collectAll[TypeDecl]
    ...
    override def fromNewNode(newNode: NewNode, mapping: NewNode => StoredNode): Unit =
        ...
        graph.indexManager.putIfIndexed(
          "FULL_NAME",
          newNode.asInstanceOf[NewType].fullName,
          this.ref
        )
end TypeDb
