...
class ClosureBinding(
  ...
  id_4762: Long /*cf https://github.com/scala/bug/issues/4762 */
    ...
    def _methodParameterInViaRefOut: overflowdb.traversal.Traversal[MethodParameterIn] =
        get()._methodParameterInViaRefOut
    ...
    /** Traverse to METHOD_REF via CAPTURE IN edge.
      */
    def _methodRefViaCaptureIn: overflowdb.traversal.Traversal[MethodRef] =
        get()._methodRefViaCaptureIn
...
class ClosureBindingDb(ref: NodeRef[NodeDb]) extends NodeDb(ref) with StoredNode
    ...
    def _localViaRefOut: Local =
        try refOut.collectAll[Local].next()
        catch
    ...
    def _methodRefViaCaptureIn: overflowdb.traversal.Traversal[MethodRef] =
        captureIn.collectAll[MethodRef]
    def _typeRefViaCaptureIn: overflowdb.traversal.Traversal[TypeRef] =
        captureIn.collectAll[TypeRef]
    ...
    def _localViaCapturedByIn: overflowdb.traversal.Traversal[Local] =
        capturedByIn.collectAll[Local]
    ...
    override def fromNewNode(newNode: NewNode, mapping: NewNode => StoredNode): Unit =
        ...
        this._closureOriginalName =
            newNode.asInstanceOf[NewClosureBinding].closureOriginalName.orNull
        this._evaluationStrategy = newNode.asInstanceOf[NewClosureBinding].evaluationStrategy
end ClosureBindingDb
