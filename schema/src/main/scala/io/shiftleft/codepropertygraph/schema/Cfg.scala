...
object Cfg extends SchemaBase:
    ...
    class Schema(builder: SchemaBuilder, methodSchema: Method.Schema, ast: Ast.Schema):
        ...
        /** Each METHOD has exactly one METHOD_RETURN (the formal return value), but each
          * METHOD_RETURN can have multiple RETURN nodes. This way, we can represent a method with
